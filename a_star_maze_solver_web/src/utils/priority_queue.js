    ...
    while(currIndex !== 0)
    {
      if(this.heap[this.heapParentIdx(currIndex)].f < this.heap[currIndex].f)
        break;
              ...
              && this.tieBreaker(this.heap[this.heapParentIdx(currIndex)].path_cost_g,this.heap[currIndex].path_cost_g))
        break;
  ...
  minHeapifyAStar = (heapIdx) => {
    ...
    if(leftIdx < this.heapSize)
    {
      if(this.heap[leftIdx].f < this.heap[heapIdx].f)
        leastIdx = leftIdx;
      else if((this.heap[leftIdx].f === this.heap[heapIdx].f) && this.tieBreaker(this.heap[leftIdx].path_cost_g,this.heap[heapIdx].path_cost_g))
        leastIdx = leftIdx;
    }
    if(rightIdx < this.heapSize)
    {
      if(this.heap[rightIdx].f < this.heap[leastIdx].f)
        leastIdx = rightIdx;
      else if((this.heap[rightIdx].f === this.heap[leastIdx].f) && this.tieBreaker(this.heap[rightIdx].path_cost_g,this.heap[leastIdx].path_cost_g))
        leastIdx = rightIdx;
    }
    if(leastIdx !== heapIdx)
    {
